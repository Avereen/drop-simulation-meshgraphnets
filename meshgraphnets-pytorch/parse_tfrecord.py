...
def load_dataset(path, split):
    """Load dataset."""
    with open(path+'/meta.json', 'r') as fp:#with open(os.path.join(path, 'meta.json'), 'r') as fp:
...
if __name__ == '__main__':
    tf.compat.v1.enable_resource_variables()#tf.enable_resource_variables()
    tf.compat.v1.enable_eager_execution#tf.enable_eager_execution()
    path = 'C:/Users/Alexander/Documents/GitHub/meshGraphNets_pytorch/data/hd5'

    tf_datasetPath='C:/Users/Alexander/Documents/GitHub/meshGraphNets_pytorch/data/cylinder_flow'
    os.makedirs(path, exist_ok=True)

    for split in ['train', 'test', 'valid']:
        ds = load_dataset(tf_datasetPath, split)
        save_path = path + split  +'.h5'
